/**
 * DataForSEO On-Page API Crawler Connector
 * 
 * Official DataForSEO API integration for Technical SEO Crawls.
 * Manages task creation, polling, result extraction, and rate-limiting.
 * Falls back to built-in HTTP parser when credentials are not configured in development.
 */

export interface DataForSEOCrawlConfig {
  target: string;
...
export interface DataForSEORawTaskResponse {
  id: string;
  status_code: number;
...
export class DataForSEOCrawler {
  ...
  /**
   * 1. Submit an asynchronous On-Page Crawl Task to DataForSEO
   */
  async submitCrawlTask(config: DataForSEOCrawlConfig): Promise<{
    task_id: string;
    target: string;
    cost: number;
    ...
    if (!this.isConfigured) {
      // In development or when credentials aren't set yet: generate synthetic task ID
    ...
    const payload = [{
      ...
      load_resources: config.load_resources ?? true,
      enable_javascript: config.enable_javascript ?? false,
      respect_robots_txt: config.respect_robots_txt ?? true,
      check_canonical: config.check_canonical ?? true,
    ...
    if (!res.ok) {
      ...
      throw new Error(`DataForSEO crawl task submission failed (${res.status}): ${errorText}`);
    }
    ...
    if (!task || task.status_code >= 40000) {
      throw new Error(task?.status_message || 'Failed to initialize DataForSEO crawl task.');
    }
  ...
  /**
   * 2. Check if a task is ready or get task summary
   */
  async getTaskSummary(taskId: string): Promise<{
    ...
    if (!res.ok) {
      throw new Error(`Failed to fetch DataForSEO task summary (${res.status})`);
    }

    const data = await res.json();
    const task = data.tasks?.[0];

    if (!task) throw new Error('DataForSEO task not found');
  ...
  /**
   * 3. Fetch detailed crawled pages for a completed task
   */
  async getPages(taskId: string, limit = 100, offset = 0): Promise<any[]> {
    ...
    if (!res.ok) throw new Error(`Failed to fetch DataForSEO crawled pages (${res.status})`);
  ...
  /**
   * 4. Fetch crawled links (internal & external)
   */
  async getLinks(taskId: string, limit = 100): Promise<any[]> {
    if (!this.isConfigured || taskId.startsWith('dfs_task_')) {
      return [];
    }
  ...
  /**
   * 5. Fetch image resources & alt text findings
   */
  async getResources(taskId: string, limit = 100): Promise<any[]> {
    if (!this.isConfigured || taskId.startsWith('dfs_task_')) {
      return [];
    }
  ...
  /**
   * Fallback simulator for development/preview mode
   */
